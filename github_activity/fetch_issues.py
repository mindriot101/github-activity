...
def query_fetch_issues():
    _op = sgqlc.operation.Operation(_schema_root.query_type, name='FetchIssues', variables=dict(name=sgqlc.types.Arg(sgqlc.types.non_null(_schema.String)), owner=sgqlc.types.Arg(sgqlc.types.non_null(_schema.String)), first=sgqlc.types.Arg(sgqlc.types.non_null(_schema.Int)), after=sgqlc.types.Arg(_schema.String)))
    _op_repository = _op.repository(name=sgqlc.types.Variable('name'), owner=sgqlc.types.Variable('owner'))
    _op_repository_issues = _op_repository.issues(first=sgqlc.types.Variable('first'), after=sgqlc.types.Variable('after'))
