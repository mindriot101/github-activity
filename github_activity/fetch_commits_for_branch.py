...
def query_fetch_commits_for_branch():
    _op = sgqlc.operation.Operation(
        ...
        variables=dict(
            ...
            branch=sgqlc.types.Arg(sgqlc.types.non_null(_schema.String)),
            first=sgqlc.types.Arg(sgqlc.types.non_null(_schema.Int)),
            after=sgqlc.types.Arg(_schema.String),
        ),
    )
    _op_repository = _op.repository(
        owner=sgqlc.types.Variable("owner"), name=sgqlc.types.Variable("name")
    )
    _op_repository_ref = _op_repository.ref(
        qualified_name=sgqlc.types.Variable("branch")
    )
    _op_repository_ref.name()
    _op_repository_ref.prefix()
    _op_repository_ref_target = _op_repository_ref.target()
    _op_repository_ref_target__as__Commit = _op_repository_ref_target.__as__(
        _schema.Commit
    )
    _op_repository_ref_target__as__Commit_history = (
        _op_repository_ref_target__as__Commit.history(
            first=sgqlc.types.Variable("first"), after=sgqlc.types.Variable("after")
        )
    )
    _op_repository_ref_target__as__Commit_history_page_info = (
        _op_repository_ref_target__as__Commit_history.page_info()
    )
    ...
    _op_repository_ref_target__as__Commit_history_edges = (
        _op_repository_ref_target__as__Commit_history.edges()
    )
    _op_repository_ref_target__as__Commit_history_edges_node = (
