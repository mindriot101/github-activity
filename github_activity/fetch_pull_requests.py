...
def query_fetch_pull_requests():
    _op = sgqlc.operation.Operation(_schema_root.query_type, name='FetchPullRequests', variables=dict(name=sgqlc.types.Arg(sgqlc.types.non_null(_schema.String)), owner=sgqlc.types.Arg(sgqlc.types.non_null(_schema.String)), after=sgqlc.types.Arg(_schema.String), first=sgqlc.types.Arg(sgqlc.types.non_null(_schema.Int))))
    _op_repository = _op.repository(name=sgqlc.types.Variable('name'), owner=sgqlc.types.Variable('owner'))
    _op_repository_pull_requests = _op_repository.pull_requests(first=sgqlc.types.Variable('first'), after=sgqlc.types.Variable('after'))
